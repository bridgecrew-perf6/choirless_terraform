module "convert_format_lambda" {
  ...
  layers = [aws_lambda_layer_version.choirlessFfProbeLayer.arn, aws_lambda_layer_version.choirlessFfmpegLayer.arn, aws_lambda_layer_version.choirlessAPILambdaLayer.arn]
  memory_size = 2048
  timeout       = 300
...
module "renderer_compositor_child_lambda"  {
  ...
  role = aws_iam_role.choirlessLambdaRole.arn
  layers = [aws_lambda_layer_version.choirlessAPILambdaLayer.arn, aws_lambda_layer_version.choirlessFfmpegLayer.arn]
  memory_size = 1024
  timeout       = 300
  efs_access_point = aws_efs_access_point.choirlessEFSAP.arn
  local_mount_path = var.mount_path
