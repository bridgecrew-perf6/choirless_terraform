...
resource "aws_lambda_function" "lambda" {
  for_each = toset([ "getUser", "getUserByEmail"]) 
...
module "getUser_method" {
  ...
  api_path_part = "getUser"
  api_method = "GET" 
  api_lambda_arn = aws_lambda_function.lambda["getUser"].arn
  api_lambda_name = aws_lambda_function.lambda["getUser"].function_name
  api_region = data.aws_region.current.name
  api_account_id = data.aws_caller_identity.current.account_id
}

module "getUserByEmail_method" {
  source = "./modules/apimethod"
  api_id = aws_api_gateway_rest_api.choirless_api.id
  api_root_resource_id = aws_api_gateway_rest_api.choirless_api.root_resource_id
  api_path_part = "getUserByEmail"             
  api_method = "GET"                 
  api_lambda_arn = aws_lambda_function.lambda["getUserByEmail"].arn
  api_lambda_name = aws_lambda_function.lambda["getUserByEmail"].function_name
  api_region = data.aws_region.current.name
  api_account_id = data.aws_caller_identity.current.account_id
}
...
resource "aws_api_gateway_deployment" "choirless_api_deployment" {
  depends_on = [module.getUser_method,module.getUserByEmail_method]
